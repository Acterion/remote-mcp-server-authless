  ...
  async init() {
    ...
    this.server.tool(
      ...
      "Create a new study item for learning with spaced repetition scheduling",
      {
        ...
        level: z
          .enum(["A1", "A2", "B1", "B2", "C1", "C2", "easy", "medium", "hard"])
        ...
        try {
          ...
          return {
            content: [
              {
                type: "text",
                text: JSON.stringify({ error: String(error), success: false }),
              },
            ],
          };
        }
    ...
    this.server.tool(
      ...
      "Update performance/recall strength for a study item and reschedule next review",
      {
        ...
        recallStrength: z
          ...
          .describe("How well the user recalled this item"),
      },
    ...
    this.server.tool(
      ...
      "Get study items that are due for review based on spaced repetition algorithm",
      {
        ...
        recallStrength: z
          .enum(["weak", "medium", "strong"])
          .optional()
          .describe("Filter by recall strength"),
      },
      async ({ userId, recallStrength }) => {
        try {
          const now = Math.floor(Date.now() / 1000);

          const conditions = [
            eq(studyItems.userId, userId),
            lte(studyItems.nextReview, now),
          ];
    ...
    this.server.tool(
      ...
      async ({ userId }) => {
        try {
          const results = await db
            .selectDistinct({ type: studyItems.type })
            .from(studyItems)
      ...
      async ({ userId }) => {
        try {
          ...
          const stats = {
            ...
            byRecallStrength: {
              weak: allItems.filter((item) => item.recallStrength === "weak")
                .length,
              medium: allItems.filter(
                (item) => item.recallStrength === "medium"
              ).length,
              strong: allItems.filter(
                (item) => item.recallStrength === "strong"
              ).length,
            },
            byLevel: {} as Record<string, number>,
            byType: {} as Record<string, number>,
            dueForReview: allItems.filter((item) => item.nextReview <= now)
              .length,
            reviewedToday: allItems.filter(
              (item) => item.lastReviewed && item.lastReviewed >= now - 86400
            ).length,
          };
